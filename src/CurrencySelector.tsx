...
export function CurrencySelector({ value, onChange }: CurrencySelectorProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="text-slate-600 dark:text-surf-300 text-sm font-medium sr-only sm:not-sr-only">{t('pricesIn')}</span>
      <div className="flex flex-wrap gap-1.5" role="group" aria-label={t('selectCurrency')}>
        {CURRENCIES.map((c) => (
          <button
            key={c.value}
            type="button"
